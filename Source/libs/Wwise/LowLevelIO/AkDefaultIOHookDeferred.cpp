...
void* AioFuncRead(void* lpParameter)
{
	...
	// Seek in the file
	if( lseek( fileno(fileHandle), pInfo->uFilePosition, SEEK_SET ) == pInfo->uFilePosition )
	{
		// Read the data
		AkUInt32 byteTransferred = (AkUInt32) read( fileno(fileHandle), pInfo->pBuffer, pInfo->uRequestedSize );
		
		if( byteTransferred == pInfo->uRequestedSize )
		{
...
void* AioFuncWrite(void* lpParameter)
{
	...
	{
		// Read the data
		AkUInt32 byteTransferred = (AkUInt32) write( fileno(fileHandle), pInfo->pBuffer, pInfo->uRequestedSize );
	...
	)
{
	...
	// Create a thread to do the async write
	AkThread thread;
	AkThreadProperties threadProp;
	AKPLATFORM::AkGetDefaultThreadProperties(threadProp);
	AKPLATFORM::AkCreateThread( &AioFuncWrite, &io_transferInfo, threadProp, &thread, (char*)"AioThread");
